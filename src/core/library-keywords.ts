/**
 * Every keyword SeleniumLibrary defines, so the generator never names one of
 * its own the same.
 *
 * Robot resolves a suite's own keywords before a library's, so a generated
 * `Click Button` whose body calls `Click Button` calls itself — recorded on
 * shoelace.style, where a button inside a shadow root had nothing to be named
 * after but its tag.
 *
 * SeleniumLibrary 6.8.0. `npm run test:libdoc` fails if this drifts.
 */
export const LIBRARY_KEYWORDS: string[] = [
  'Add Cookie',
