...
A different job from the forum post: nobody there is looking for a test
recorder, and most of the people who react will never install it. What it is
for is the people who already know you seeing that you shipped something —
worth having when a job or an introduction comes up later.

Attach `~/Desktop/record/zelector-demo.mp4` as native video, not the GIF and
not a YouTube link. LinkedIn is inconsistent with animated GIFs and suppresses
posts that send people off the platform; native video autoplays in the feed,
which is the whole point — the last five seconds are a terminal going green.

```
Record what you do on a page, get a Robot Framework test that actually runs — no edits.

Most recorders hand you Sleep 2 and nth-child selectors, and you end up rewriting every line before it can be committed. At that point writing the test by hand is faster.

So I spent two weeks on the two things that decide whether a recorded test survives:

• Waits — inferred from what the page actually did after each action: a spinner that came and went, a URL that changed, an element that arrived. Every wait shows the reason it is there, and can be edited before you export.

• Locators — every candidate scored, with the reason for each point lost. Classes generated by a build tool, ids regenerated on every render, classes that name a state rather than an element: marked, rather than handed over as though they were stable.

The last two days went into recording eleven real sites and running whatever came out. That found twenty bugs in my own tool — three of them cases where the recording came out completely empty with no error at all: native dialogs, rich text editors, and drag and drop that the browser runs itself.

The clip is DemoQA's practice form end to end: record, download, run, green. 87 keywords, headless, nothing edited in between.

Free, MIT, no account, no servers, no data collected.
🔗 chromewebstore.google.com/detail/ligmapkiobcnblnlpomoffmobhfobdkf

#RobotFramework #TestAutomation #QA #Selenium #WebTesting
```
